...
const builtInCssClasses: Readonly<CheckboxCssClasses> = {
  ...
  input:
    "w-3.5 h-3.5 form-checkbox cursor-pointer border border-gray-300 rounded-sm text-primary focus:ring-primary",
  ...
  tooltip:
    "absolute z-10 left-0 -top-0.5 whitespace-nowrap rounded shadow-lg p-3 text-sm bg-neutral-dark text-white",
};
...
export function CheckboxOption(props: CheckboxOptionProps): JSX.Element | null {
  ...
  const existingStoredFilter = findSelectableFieldValueFilter(
    optionFilter,
    filters
  );
  ...
  useEffect(() => {
    ...
    selectFilter,
    selectedByDefault,
    existingStoredFilter,
    optionFilter,
  ]);

  const isSelected = existingStoredFilter
